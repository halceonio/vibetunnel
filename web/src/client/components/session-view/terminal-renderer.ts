...
export class TerminalRenderer extends LitElement {
  ...
  @property({ type: Object }) onTerminalHistoryBootstrap?: (e: CustomEvent) => void;
    ...
    if (this.useBinaryMode) {
      return html`
        <vibe-terminal-binary
          ...
          @terminal-history-bootstrap=${(e: Event) => this.handleTerminalHistoryBootstrap(e)}
        ></vibe-terminal-binary>
      `;
    } else {
      return html`
  ...
  private handleTerminalHistoryBootstrap(e: Event) {
    this.onTerminalHistoryBootstrap?.(e as CustomEvent);
  }
}
